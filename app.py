...
from census.entity.census_predictor import census_Predictor, census_Data
...
MEDIAN_CENSUS_VALUE_KEY = "median_salary_value"
...
def predict():
    ...
    if request.method == 'POST':
        ...
        workclas = object(request.form['workclas'])
        census_median_age = float(request.form['census_median_age'])
        ...
        relationship = object(request.form['relationship'])
        race = object(request.form['race'])
        sex = object(request.form['sex'])
        capital_gain = int(request.form['capital_gain'])
        capital_loss = int(request.form['capital_loss'])
        hours_per_week = int(request.form['hours_per_week'])
        native_country = object(request.form['native_country'])
        wages = object(request.form['wages'])

        census_data = census_Data(age=age,
                                   workclas=workclas,
                                   census_median_age=census_median_age,
                                   fnlwgt=fnlwgt,
        ...
        census_predictor = census_Predictor(model_dir=MODEL_DIR)
        median_census_value = census_predictor.predict(X=income_prediction_df)
        context = {
